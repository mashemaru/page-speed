		...
		switch ($type)
		{
			...
			case 'style':
				if (!is_admin() && !is_login_page()) {
					$return = "<script>loadCSS('" . esc_url($string) . "',0,'" . esc_attr($media) . "');</script><noscript><link rel='stylesheet' id='"
						. esc_attr($group_handle) . "-group-css' href='"
						. esc_url($string) . "' type='text/css' media='"
						. esc_attr($media) . "' /></noscript>\r\n";

					if ($title)
					{
						$return = "<script>loadCSS('" . esc_url($string) . "',0,'" . esc_attr($media) . "');</script><noscript><link rel='alternate stylesheet' id='"
